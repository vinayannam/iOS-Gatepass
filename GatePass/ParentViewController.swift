...
class ParentViewController: UICollectionViewController,UICollectionViewDelegateFlowLayout{
    ...
    override func viewDidLoad() {
        ...
        do{
            ...
            for pass in searchResults as [Passes]{
                if(pass.username == student && pass.p_request == false){
                    var check = false;
        ...
        collectionView?.register(ParentPassCell.self, forCellWithReuseIdentifier: cellId)
        
    }
    
    
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let pass: ParentPassCell
        pass = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! ParentPassCell
    ...
    @objc func acceptAction(sender:UIButton){
        ...
        do{
            let searchResults = try DatabaseController.persistentContainer.viewContext.fetch(fetchRequest)
            for pass in searchResults as [Passes]{
                if(pass.username == student && pass.id == Int16(sender.tag) && pass.p_request == false){
                    pass.p_request = true
